...
//        We check whether the city entered by the user is in the list
            ...
            if (!isCorrectCity){
                System.out.println("Такого міста не існує в моїй базі даних. Будьласка, введіть інше місто");
            }

//        We check whether we are entering the word from the right letter
            if(lastComputerCity != null){

                char lastComputerCityChar = lastComputerCity.charAt(lastComputerCity.length() - 1);
                if ((Character.toLowerCase(lastComputerCityChar)  == 'ь') || (Character.toLowerCase(lastComputerCityChar)  == 'и')){
                ...
                char firstInputChar = input.charAt(0);
                    if (Character.toLowerCase(lastComputerCityChar) != Character.toLowerCase(firstInputChar)){
                        System.out.println("Ви ввели місто з непраильною першою літерою");
                        continue;
                    }
